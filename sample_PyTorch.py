...
if __name__ == '__main__':
  ...
  def train_step(x,t):
    model.train()     # modelを学習モードにする。 ←→ 推論モード: model.eval()
    preds = model(x)
  ...
  for epoch in range(epochs):
    ...
    x_ = torch.Tensor(x_).to(device)    # Tensor型に変換: 明示的に変換しないといけない。
    t_ = torch.Tensor(t_).to(device)    # Tensor型に変換: 明示的に変換しないといけない。

    for n_batch in range(n_batches):
      start = n_batch * batch_size
      end = start + batch_size
      loss = train_step(x_[start:end],t_[start:end])
      train_loss += loss.item()     # loss.item():  lossの数値だけを抽出。
  ...
  def test_step(x,t):
    ...
    model.eval()  #推論モードへ。
    preds = model(x)        # preds: 勾配計算の情報を含むテンソル型
    loss = compute_loss(t,preds)
    return loss, preds

  loss, preds = test_step(x_test,t_test)
  test_loss = loss.item()
  preds = preds.data.cpu().numpy() > 0.5      # numpy配列に変換。
